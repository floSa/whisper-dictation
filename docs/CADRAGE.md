...
1. **Déclenchement instantané** par raccourci clavier global depuis n'importe quelle application (VS Code, navigateur, traitement de texte).
...
- Capture audio microphone haute fidélité (16 kHz mono) en mémoire tampon.
...
- Raccourci global configurable (mode bascule *toggle* et *push-to-talk*).
- Injection automatique du texte par simulation de frappe/presse-papier au niveau du curseur.
- Scripts de gestion du cycle de vie du conteneur pour libérer la VRAM GPU.
...
| Raccourcis clavier | `pynput` | LGPL-3.0 |
...
**Décisions figées**
- **Client Python léger avec `uv`** : Choisi **plutôt que** l'application commerciale Superwhisper, **parce que** Superwhisper impose un abonnement payant pour les modèles lourds, cloisonne ses modèles et refuse le raccordement direct au serveur Docker GPU local existant.
...
- **Gestionnaire ON/OFF de la VRAM** : Choisi **plutôt que** de laisser Whisper charger en continu, **parce que** l'utilisateur doit pouvoir récupérer 100% de la mémoire GPU (16 Go) pour lancer des jeux vidéo ou de volumineux modèles LLM locaux.
...
0. **Socle V1** : Capture audio, client HTTP, raccourci global, injection texte, scripts VRAM ON/OFF.
...
- **Tests d'intégration manuels** :
  - Démarrage et arrêt du conteneur Docker.
